...
class Register extends Component {
    ...
    //         try {
    //             if (message) {
    //                  const messageArray = message.split(' ')[0]
    //                  console.log("MESSAGE::::", message)
    //             }
    //         }catch(error){
    ...
    //     })
    // }).catch(error=> {
    //  console.log("ERROR IN 2nd::::", error)

    //  logErrorWithMessage(
    //      error.message,
    //      'Error in second block'
    //  )
    // })
    ...
    uploadImageToFirebase = async (imgUri) => {
        this.setState({ isImgLoading: true, isLoading: true })
        const uri = imgUri;
    ...
    checkUserPic = async (url) => {
        await axios.post(`${BASE_URL}${CHECK_FACE}`, {
            ...
            console.log('checkuser face:', error)
        })
    }
    ...
    step3 = () => (
        <KeyboardAwareScrollView keyboardShouldPersistTaps='always'>
            <Animatable.View animation="fadeInUpBig" style={{ flex: 1, marginTop: 20, alignItems: 'center' }}>
                ...
                <CustomInput hint='Enter your email address' value={this.state.email} keyboardType='email-address' maxLength={30}
                    onChange={(val) => this.setState({ email: val })} icon='mail' secureTextEntry={false}
                    marginTop={10} />

                <CustomButton title='Proceed' titleColor={COLORS.white} backgroundColor={COLORS.themeColor}
                disabled={
                    this.state.f_name === '' || this.state.f_name.length < 3 ||
                        this.state.l_name === '' || this.state.l_name.length < 3 ||
                        this.state.email === '' || this.state.email.length < 6 ? true : false
                }
    ...
    step4 = () => (
        ...
        <Animatable.View animation="fadeInUpBig" style={styles.container}>
            <View style={{ flex: 1, marginTop: 20, alignItems: 'center' }}>
                ...
                <CustomButton title='Confirm' titleColor={COLORS.white} backgroundColor={COLORS.themeColor} disabled={this.state.pin.length < 6 || this.state.confirmPin.length < 6 ? true : false} callFunction={() => {
    ...
    render() {
        ...
        return (
            <KeyboardAwareScrollView keyboardShouldPersistTaps='always' contentInset={{top: 0 , left: 0, right: 0, bottom: 50}} style={styles.container}>
